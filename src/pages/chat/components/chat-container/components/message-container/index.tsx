...
import {
  ChannelMessage,
  DMMessage,
  GetAllDMMessagesResponse,
  GetChannelMessagesResponse
} from '@/types/get-all-messages.type';

const isDMMessage = (message: DMMessage | ChannelMessage): message is DMMessage => {
  return typeof message.sender === 'string'; // Since DMMessage.sender is a string
};
...
const MessageContainer = () => {
  ...
  useEffect(() => {
    const getAllMessages = async () => {
      try {
        if (selectedChatData) {
          const response = await apiClient.post<GetAllDMMessagesResponse>(
    ...
    const getChannelMessages = async () => {
      try {
        if (selectedChatData) {
          const response = await apiClient.get<GetChannelMessagesResponse>(
    ...
    return selectedChatMessages.map((message, index) => {
      ...
      return (
        <div key={index}>
          ...
          {selectedChatType === 'contact' && isDMMessage(message) && renderDMMessages(message)}
  ...
  const renderDMMessages = (message: DMMessage) => (
    <>
      {selectedChatData && (
            ...
            >
              {checkIfImage(message?.fileUrl ?? '') ? (
                ...
                <div className="flex items-center justify-center gap-4">
                  ...
                  <span>{message.fileUrl && message.fileUrl.split('/').pop()}</span>
                  <span
      ...
      <>
        {userInfo && (
          ...
          >
            {message.messageType === 'text' && (
              <div
                className={`${message.sender._id === userInfo.id ? 'bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50' : 'bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20'} border inline-block p-4 rounded my-1 max-w-[50%] break-words ml-9`}
              >
                {message.content}
              </div>
            )}
            {message.messageType === 'file' && (
              <div
                className={`${message.sender._id === userInfo.id ? 'bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50' : 'bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20'} border inline-block p-4 rounded my-1 max-w-[50%] break-words`}
              >
                {checkIfImage(message.fileUrl ?? '') ? (
                  <div
                    className="cursor-pointer"
            ...
            {message.sender._id !== userInfo.id ? (
              <div className="flex items-center justify-start gap-3">
                <Avatar className="h-8 w-8 rounded-full overflow-hidden">
                  ...
                  <AvatarFallback
                    className={`uppercase h-8 w-8 text-lg flex items-center justify-center rounded-full ${getColors(message.sender.color)}`}
                  >
